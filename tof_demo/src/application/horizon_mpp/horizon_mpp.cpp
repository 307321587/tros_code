...
static int hb_enable_sensor_clk(uint32_t mipiIdx)
{
	...
	}else if (0 == board_type)
	{
		printf("000000\n");
		(void)system("echo 114 > /sys/class/gpio/export");
		(void)system("echo out > /sys/class/gpio/gpio114/direction");
		(void)system("echo 0 > /sys/class/gpio/gpio114/value");
		(void)system("sleep 0.2");
		(void)system("echo 1 > /sys/class/gpio/gpio114/value");
		(void)system("echo 114 > /sys/class/gpio/unexport");
		(void)system("echo 1 > /sys/class/vps/mipi_host1/param/stop_check_instart");
	} 
	else {

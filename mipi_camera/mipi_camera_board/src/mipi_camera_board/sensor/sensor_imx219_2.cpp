...
/** 2464P **/
MIPI_SENSOR_INFO_S SENSOR_2LANE_IMX219_15FPS_2464P_10BIT_LINEAR_INFO_2 =
{
	.deseEnable = 0, // 该 sensor 是否有 serdes（串解器）
	.inputMode = INPUT_MODE_MIPI, // sensor 接入方式,mipi还是dvp
	.sensorInfo = {
...
MIPI_ATTR_S MIPI_2LANE_SENSOR_IMX219_15FPS_2464P_10BIT_LINEAR_ATTR_2 =
{
    .mipi_host_cfg = {
        .lane = 2, // 硬件上sensor用了几个mipi数据lane，f37用的1 lane, os8a10 4K的用了4 lane
        .datatype = 0x2b, // sensor的输出数据类型，请参考 《X3J3平台AIOT媒体系统接口手册.pdf》第3.5.36节 DATA TYPE
...
VIN_DEV_ATTR_S DEV_ATTR_IMX219_15FPS_2464P_LINEAR_BASE_2 = {
    .stSize = { // 输入的数据
		.format = 0, // 像素格式， format 为 0 代表是raw, 根据 pixel_lenght 来表示究竟是 raw8, raw12 还是 raw16 。
		.width = 3264, // 数据宽
		.height = 2464, // 数据高
...
VIN_PIPE_ATTR_S PIPE_ATTR_IMX219_15FPS_2464P_LINEAR_BASE_2 = {
	.ddrOutBufNum = 6, // ISP->GDC ddr的buf数量，最少可以设置成 2
	.snsMode = SENSOR_NORMAL_MODE, // sensor 工作模式，要和MIPI_SENSOR_INFO_S配置的sensor_mode一致
	.stSize = { // 数据格式同 VIN_DEV_SIZE_S
		.format = 0,
...
VIN_DIS_ATTR_S DIS_ATTR_IMX219_15FPS_2464P_LINEAR_BASE_2 = {
	.picSize = { // 输入数据宽高
...
VIN_LDC_ATTR_S LDC_ATTR_IMX219_15FPS_2464P_LINEAR_BASE_2 = {
  .ldcEnable = 0, // LDC 是否使能
  .ldcPath = { // 输出类型
	 .rg_y_only = 0, // ???
	 .rg_uv_mode = 0, // ???
...
void setImx219_2(SensorSetting& sensor_setting)
{
	printf("set_sensor_imx219_param\n");
	/*定义 sensor   初始化的属性信息 */
	sensor_setting.snsinfo = SENSOR_2LANE_IMX219_15FPS_2464P_10BIT_LINEAR_INFO_2;
	/*定义 mipi 初始化参数信息 */
	sensor_setting.mipi_attr = MIPI_2LANE_SENSOR_IMX219_15FPS_2464P_10BIT_LINEAR_ATTR_2;
	/*定义 dev 初始化的属性信息 */
	sensor_setting.devinfo = DEV_ATTR_IMX219_15FPS_2464P_LINEAR_BASE_2;
	/*定义 pipe 属性信息 */
	sensor_setting.pipeinfo = PIPE_ATTR_IMX219_15FPS_2464P_LINEAR_BASE_2;
	/*定义 dis 属性信息 */
	sensor_setting.disinfo = DIS_ATTR_IMX219_15FPS_2464P_LINEAR_BASE_2;
	/*定义 ldc 属性信息 */
	sensor_setting.ldcinfo = LDC_ATTR_IMX219_15FPS_2464P_LINEAR_BASE_2;
	// return sensor_sif_dev_init(multi_sensors[1],1);
}